...
### Bearer-token theft-and-replay: token-binding evaluated, not adopted

Every leg carries the AS-issued access token as a plain bearer credential (lines 5, 7, 228) — possession alone is sufficient to use it, with no mechanism binding it to the client it was issued to. This matters because the design already treats MCP servers as adversarial on the response path (see "DLP on MCP tool responses" below): a downstream MCP server sits on the same channel the bearer token's authority travels through, so a token leaked or replayed via that channel is fully usable by whoever holds it.

**Scope decision: not adopted this design phase.** DPoP (RFC 9449) and mTLS (RFC 8705) were both considered as token-binding mechanisms to close that gap. Neither is adopted for this design phase (POC). DPoP requires every caller — CLI, IDE, Claude Code, agent runtimes (line 13) — to generate a proof-of-possession key and sign every request, and mTLS requires client-certificate issuance and rotation infrastructure this design doesn't build; both are real client-side implementation lifts, unlike the RS256/JWKS verifier above, which is a single function reusing an existing pattern. This is a POC-scope and client-complexity decision, not a claim that Zitadel lacks DPoP or mTLS support — that is not asserted either way here.

This is a distinct decision from the mTLS/workload-identity deferral in "Per agent type, not per instance" (line 333) and "Instance-bound attestation for machine-user credentials" (line 504): those cover binding the machine-user **client-credential secret** used in the RFC 8693 `actor_token` exchange to a single workload instance. This section covers the AS-issued **access token** used across all four legs — a different token, evaluated here on its own terms, not restated from those sections.
...
**Resource-string canonicalization: `context.resource` is normalized before it reaches `glob.match`, split across two components because Rego cannot do all of it alone.** `tool.arguments` itself is never pattern-matched — only `context.resource` feeds `glob.match`, against `resource_pattern`, so canonicalization is scoped to that one field, not to `tool.arguments` generally. Without it, an allow-list entry like `repo:org/*` could be bypassed by a caller sending `REPO:ORG/Name/` or a Unicode-confusable lookalike that a naive string comparison treats as distinct from the canonical form. Two normalization steps are enforced inside `policies/mcp/authz.rego` itself, in a `canonicalize()` helper applied to both `context.resource` and `resource_pattern` immediately before `glob.match`: case normalization (lowercasing) and trailing-slash stripping. A third step, Unicode normalization (NFC), cannot be expressed this way — OPA has no Unicode-normalization builtin — so it is instead the MCP Reverse Proxy's responsibility, applied to `context.resource` in Python (`unicodedata.normalize("NFC", s)`) before the OPA sidecar input above is constructed; this is a documented pre-POC design decision, not yet running code, consistent with the proxy's overall not-yet-built status noted above. `policies/mcp/authz_test.rego`'s `test_resource_pattern_match_allow_case_and_trailing_slash_insensitive` is the executable proof that a differently-formatted but equivalent resource string (`REPO:ORG/Name/` against the `repo:org/*` pattern) still resolves to the same allow decision as the canonical form.
...
**What this does and does not deliver.** The `INSERT`-only grant, forced RLS, and `deny_audit_mutation` trigger prevent retroactive `UPDATE`/`DELETE`/`TRUNCATE` of existing rows — rewriting history — including by the `gateway_app` credential holder itself. They do not prevent a compromised Governance Service, the sole holder of that `INSERT` credential, from fabricating new, plausible-looking rows: inserting false history, not just being blocked from erasing true history. Closing that gap requires hash-chaining or WORM storage (see "Open questions / deferred past POC" below), not a broader grant restriction.
...
Controls #1 and #2 only answer **who can trigger the workflow** and **which commit runs.** Neither constrains what that commit's workflow file and package manifest actually publish once it runs:

- **Package name / version — unconstrained.** The Gateway never parses or validates `package.json`/`pyproject.toml`/`Cargo.toml` or any other manifest; once control #2 confirms the correct SHA is running, whatever that commit's manifest encodes is what gets published. Name/version are gated only transitively, by this repo's ordinary code-review and branch-protection process for merging to a tag-eligible ref — not by any Gateway-side check.
- **Target registry — partially constrained.** The protected Environment's OIDC trust relationship is scoped to a specific registry per Environment (e.g. a `publish-registry` Environment trusted only by npm's OIDC config), so a run can only reach the registry(ies) named in that trust policy. Nothing, however, stops a modified workflow file from adding a step that runs under that same Environment and publishes a *different* package to that same registry.
- **Accepted residual risk.** Publish-parameter review is an accepted residual risk for POC: the Gateway's authorization boundary ends at "this exact reviewed commit's workflow ran" and does not extend to validating what that workflow does. A compromised or insufficiently-reviewed change to the workflow file or package manifest, merged before the tag/dispatch, would publish under the Gateway's audit trail without the Gateway having reviewed — or being able to review — the actual artifact, name, or version. The "Ingesting GitHub Actions run and re-run events" plan under "Open questions" below is the detection counterpart: it records that a publish happened and links it to the trigger's audit row, but it does not validate publish parameters either.
...
- **WORM storage / full SIEM integration / ISO 27001 3-year retention** — reasonable to defer past POC per the compliance research; noted so it isn't forgotten. This is also the control that would close the fabricated-row gap the INSERT-only grant leaves open (see "Write-access model" above): hash-chaining or WORM storage would let a fabricated row be detected after the fact, which the grant alone cannot do.
